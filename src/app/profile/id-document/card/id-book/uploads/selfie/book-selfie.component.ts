...
@Component({
  selector: 'app-book-selfie',
  templateUrl: './book-selfie.component.html',
  styleUrls: ['./book-selfie.component.css']
})
export class BookSelfieComponent implements OnInit, OnDestroy {
  image: any;
  fileType: string;
  txID: string;
  ...
  getPreview() {
    this.imageSub = new Observable<string>(observer => {
      ...
      this.igmService.getPreview(DocumentTypeEnum.ID_BOOK, DocumentFileTypeEnum.SELFIE, 50).subscribe(blob => {
        ...
        console.log('Failed to get selfie preview of ID-Book : ' + error.message);
      });
    }).subscribe((result) => {
      this.image = result;
